...
namespace Microsoft.Boogie.Houdini
{
  public class AbsHoudini
  {
    ...
    public AbsHoudini(Program program, IAbstractDomain defaultElem)
    {
      ...
      this.impl2ErrorHandler =
        new Dictionary<string, Tuple<ProverInterface.ErrorHandler, AbsHoudiniCounterexampleCollector>>();
      ...
      existentialFunctions.Values.Iter(func =>
      {
        ...
        if (func.Body != null)
          throw new AbsHoudiniInternalError(string.Format("Existential function {0} should not have a body",
            func.Name));
    ...
    public VCGenOutcome ComputeSummaries()
    {
      ...
      while (worklist.Any())
      {
        ...
        foreach (var tup in impl2FuncCalls[impl])
        {
          ...
          if (controlVar.InParams.Count != 0)
          {
            term = new ForallExpr(Token.NoToken, new List<Variable>(controlVar.InParams.ToArray()),
              new Trigger(Token.NoToken, true,
                new List<Expr> {new NAryExpr(Token.NoToken, new FunctionCall(controlVar), args)}),
              term);
          }
        ...
        if (proverOutcome == ProverInterface.Outcome.TimeOut || proverOutcome == ProverInterface.Outcome.OutOfMemory ||
            proverOutcome == ProverInterface.Outcome.OutOfResource)
        {
    ...
    private List<Tuple<string, List<Model.Element>>> ExtractState(string impl, Counterexample error)
    {
      ...
      {
        failingAssert = lastBlock.Cmds.OfType<AssertCmd>()
          .FirstOrDefault(ac => ac == assertCounterexample.FailingAssert);
      }
    ...
    private List<Tuple<string, List<Model.Element>>> ExtractState(string impl, Expr expr, Model model)
    {
      ...
      foreach (var tup in funcsUsed.Where(t => t.Item2 != null))
      {
        ...
        foreach (var f in boundExpr.Dummies.OfType<Variable>())
          formalToConstant.Add(f.Name,
            new Constant(Token.NoToken,
              new TypedIdent(Token.NoToken, f.Name + "@subst@" + (existentialConstCounter++), f.TypedIdent.Type),
              false));
        be = Substituter.Apply(
          new Substitution(v =>
            formalToConstant.ContainsKey(v.Name) ? Expr.Ident(formalToConstant[v.Name]) : Expr.Ident(v)), be);
        ...
        {
          var arg = Substituter.Apply(
            new Substitution(v =>
              formalToConstant.ContainsKey(v.Name) ? Expr.Ident(formalToConstant[v.Name]) : Expr.Ident(v)), funcArg);
    ...
    private void GenVC(Implementation impl)
    {
      ...
      VCExpr controlFlowFunctionAppl = gen.ControlFlowFunctionApplication(gen.Integer(Microsoft.Basetypes.BigNum.ZERO),
        gen.Integer(Microsoft.Basetypes.BigNum.ZERO));
      VCExpr eqExpr = gen.Eq(controlFlowFunctionAppl,
        gen.Integer(Microsoft.Basetypes.BigNum.FromInt(impl.Blocks[0].UniqueId)));
      vcexpr = gen.Implies(eqExpr, vcexpr);

      ProverInterface.ErrorHandler handler = new VCGen.ErrorReporter(gotoCmdOrigins, label2absy, impl.Blocks,
        vcgen.incarnationOriginMap, collector, mvInfo, prover.Context, program);
      ...
      Macro macro = new Macro(Token.NoToken, impl.Name + "Macro", new List<Variable>(),
        new Formal(Token.NoToken, new TypedIdent(Token.NoToken, "", Bpl.Type.Bool), false));
        ...
        for (int i = boundVars.Count - 1; i >= 1; i--)
        {
          if (boundVars[i].ContainsKey(node.Name))
            throw new AbsHoudiniInternalError(
              "Existential functions can only refer to outermost bound variables in an expression");
        }
      }
  ...
  public class PowDomain : IAbstractDomain
  {
    ...
    bool isBottom
    {
      get { return tlevel == Val.FALSE; }
    }

    bool isTop
    {
      get { return tlevel == Val.TRUE; }
    }
    ...
    Expr IAbstractDomain.Gamma(List<Expr> vars)
    {
      ...
      if (v.Type.IsBv)
      {
        ...
        return new NAryExpr(Token.NoToken, new FunctionCall(bvslt), new List<Expr>
        {
          v,
  ...
  {
    // Type name -> Instance
    private static Dictionary<string, IAbstractDomain> abstractDomainInstances =
      new Dictionary<string, IAbstractDomain>();

    private static Dictionary<string, IAbstractDomain> abstractDomainInstancesFriendly =
      new Dictionary<string, IAbstractDomain>();
  ...
  public class AbstractHoudini
  {
    ...
    public AbstractHoudini(Program program)
    {
      ...
      this.vcgen = new VCGen(program, CommandLineOptions.Clo.ProverLogFilePath,
        CommandLineOptions.Clo.ProverLogFileAppend, new List<Checker>());
      this.prover = ProverInterface.CreateProver(program, CommandLineOptions.Clo.ProverLogFilePath,
        CommandLineOptions.Clo.ProverLogFileAppend, -1);
    ...
    public void computeSummaries(ISummaryElement summaryClass)
    {
      ...
      while (worklist.Any())
      {
        ...
        if (changed)
        {
          Pred[impl].Where(pred => UseBilateralAlgo || pred != impl)
            .Iter(pred => worklist.Add(Tuple.Create(impl2Priority[pred.Name], pred)));
        }
      }
    ...
    public HashSet<string> GetPredicates()
    {
      ...
      prover = ProverInterface.CreateProver(program, CommandLineOptions.Clo.ProverLogFilePath,
        CommandLineOptions.Clo.ProverLogFileAppend, -1);
    ...
    public Expr GetSummary(Program program, Procedure proc)
    {
      ...
      return impl2Summary[proc.Name].GetSummaryExpr(
        v =>
        {
          if (vars.ContainsKey(v)) return vars[v];
          else return null;
        },
        v =>
        {
          if (vars.ContainsKey(v)) return new OldExpr(Token.NoToken, vars[v]);
          else return null;
        });
    }
        ...
        foreach (var blk in impl.Blocks)
        {
          foreach (var cmd in blk.Cmds.OfType<AssumeCmd>())
          {
            ...
            cmd.Expr = impl2Summary[calleeName].GetSummaryExpr(
              v =>
              {
                if (always.ContainsKey(v)) return always[v];
                else return null;
              },
              v =>
              {
                if (forold.ContainsKey(v)) return forold[v];
                else return null;
              });
          }
        }
    ...
    private bool ProcessImpl(Implementation impl)
    {
      ...
      while (true)
      {
        ...
        if (UseBilateralAlgo)
        {
          if (proverOutcome == ProverInterface.Outcome.TimeOut ||
              proverOutcome == ProverInterface.Outcome.OutOfMemory ||
              proverOutcome == ProverInterface.Outcome.OutOfResource)
          {
            if (CommandLineOptions.Clo.Trace)
              Console.WriteLine("Timeout/Spaceout while verifying " + impl.Name);
  ...
  public class ConstantVal : ISummaryElement
  {
    ...
    public VCExpr GetSummaryExpr(Dictionary<string, VCExpr> incarnations, VCExpressionGenerator gen)
    {
      ...
      foreach (var v in vars)
      {
        ...
        consts.Iter(c =>
          vexpr = gen.OrSimp(vexpr, gen.Eq(incarnations[v.Name], gen.Integer(Microsoft.Basetypes.BigNum.FromInt(c)))));
  ...
  public class PredicateAbs : ISummaryElement
  {
    ...
    private Model.Element Eval(Expr expr, Dictionary<string, Model.Element> state)
    {
      ...
      if (expr is NAryExpr)
      {
        ...
        {
          return ToElem((nary.Fun as BinaryOperator).Evaluate(ToValue(Eval(nary.Args[0], state)),
            ToValue(Eval(nary.Args[1], state))));
        }
    ...
    public static void FindUnsatPairs(VCExpressionGenerator gen, ProverInterface prover)
    {
      ...
      predMaps.Add(PrePreds);
      predMaps.Add(PostPreds);
    ...
    private object ToValue(Model.Element elem)
    {
          ...
          (elem as Model.DatatypeValue).Arguments[0] is Model.Integer)
      {
        ...
        return Microsoft.Basetypes.BigNum.FromInt(-1 * ((elem as Model.DatatypeValue).Arguments[0] as Model.Integer)
          .AsInt());
      }
      ...
      if (expr is NAryExpr)
      {
        ...
        if (nary.Fun is UnaryOperator)
        {
          if ((nary.Fun as UnaryOperator).Op == UnaryOperator.Opcode.Not)
            ...
            return gen.Function(VCExpressionGenerator.SubIOp, gen.Integer(Basetypes.BigNum.FromInt(0)),
              ToVcExpr(nary.Args[0], incarnations, gen));
          else
        ...
        if (nary.Fun is BinaryOperator)
        {
          return gen.Function(Translate(nary.Fun as BinaryOperator), ToVcExpr(nary.Args[0], incarnations, gen),
            ToVcExpr(nary.Args[1], incarnations, gen));
        }
    ...
    public void Simplify()
    {
      ...
      if (value[findex].isTrue)
      {
        ...
        for (int i = 0; i < value.Length; i++)
          if (i != findex)
            value[i] = new PredicateAbsDisjunct(false, procName);
        return;
      }
    ...
    public ISummaryElement GetTrueSummary(Program program, Implementation impl)
    {
      ...
      for (int i = 0; i < PostPreds[this.procName].Count; i++)
        ret.value[i] = new PredicateAbsDisjunct(false, impl.Name);
    ...
    public void Join(Dictionary<string, Model.Element> state, Model model)
    {
      ...
      for (int i = 0; i < PostPreds[procName].Count; i++)
      {
        ...
        if (!postPredsVal[i])
        {
          newDisj = new PredicateAbsDisjunct(indexSeq.Where(j => !prePredsVal[j]),
            indexSeq.Where(j => prePredsVal[j] && !PosPrePreds[procName].Contains(j)), procName);
        }
    ...
    public VCExpr GetSummaryExpr(Dictionary<string, VCExpr> incarnations, VCExpressionGenerator gen)
    {
      ...
      {
        ret = gen.AndSimp(ret,
          gen.ImpliesSimp(value[i].ToVcExpr(j => ToVcExpr(PrePreds[procName][j].expr, incarnations, gen), gen),
            ToVcExpr(PostPreds[procName][i].expr, incarnations, gen)));
      }
    ...
    public Expr GetSummaryExpr(Func<string, Expr> always, Func<string, Expr> forold)
    {
      ...
      {
        ret = Expr.And(ret,
          Expr.Imp(value[i].ToExpr(j => ToExpr(PrePreds[procName][j].expr, always, forold)),
            ToExpr(PostPreds[procName][i].expr, always, forold)));
      }
  ...
  public class PredicateAbsConjunct
  {
    ...
    {
      if (posPreds.Intersect(negPreds).Any() || negPreds.Intersect(posPreds).Any() ||
          (posPreds.Count + negPreds.Count > ConjunctBound))
      {
        isFalse = true;
    ...
    public Expr ToExpr(Func<int, Expr> predToExpr)
    {
      if (isFalse) return Expr.False;
      Expr ret = Expr.True;
      var pp = posPreds.ToList();
      pp.Sort();
      var np = negPreds.ToList();
      np.Sort();
  ...
  class FindSummaryPred : MutatingVCExprVisitor<bool>
  {
      ...
      bool arg)
    {
      ...
      summaryPreds.Add(Tuple.Create(
        calleeName.Substring(0, calleeName.Length - AbstractHoudini.summaryPredSuffix.Length), false, controlConst,
        retnary));
