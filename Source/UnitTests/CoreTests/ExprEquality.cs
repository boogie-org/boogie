...
namespace CoreTests
{
  ...
  public class ExprEquality
  {
    ...
    public void LiteralBool()
    {
      ...
      Assert.AreEqual(constant.GetHashCode(),
        constant2.GetHashCode()); // If the .Equals() is true then hash codes must be the same
    }
    ...
    public void SimpleAdd()
    {
      ...
      Assert.AreEqual(add.GetHashCode(),
        add2.GetHashCode()); // If the .Equals() is true then hash codes must be the same
    }

    [Test()]
    public void SimpleIdentifierExprs()
    {
      var variable = new GlobalVariable(Token.NoToken,
        new TypedIdent(Token.NoToken, "foo", Microsoft.Boogie.Type.GetBvType(8)));
      ...
      Assert.AreEqual(idVar.GetHashCode(),
        idVar2.GetHashCode()); // If the .Equals() is true then hash codes must be the same
    }
    ...
    public void SimpleIdentifierVariables()
    {
      var variable = new GlobalVariable(Token.NoToken,
        new TypedIdent(Token.NoToken, "foo", Microsoft.Boogie.Type.GetBvType(8)));
      var variable2 = new GlobalVariable(Token.NoToken,
        new TypedIdent(Token.NoToken, "foo", Microsoft.Boogie.Type.GetBvType(8)));
      var idVar = new IdentifierExpr(Token.NoToken, variable);
    ...
    public void SimpleBVConcat()
    {
      ...
      Assert.AreEqual(concat.GetHashCode(),
        concat2.GetHashCode()); // If the .Equals() is true then hash codes must be the same
    }
    ...
    public void SimpleBVExtract()
    {
      ...
      Assert.AreEqual(extract.GetHashCode(),
        extract2.GetHashCode()); // If the .Equals() is true then hash codes must be the same
    }
    ...
    public void SimpleForAll()
    {
      ...
      Assert.AreEqual(forall.GetHashCode(),
        forall2.GetHashCode()); // If the .Equals() is true then hash codes must be the same
    }
    ...
    public void SimpleExists()
    {
      ...
      Assert.AreEqual(exists.GetHashCode(),
        exists2.GetHashCode()); // If the .Equals() is true then hash codes must be the same
    }
    ...
    public void SimpleLambda()
    {
      ...
      var lambdaExpr =
        new LambdaExpr(Token.NoToken, new List<TypeVariable>(), new List<Variable>() {boundVar}, null, id);

      var id2 = new IdentifierExpr(Token.NoToken, boundVar);
      var lambdaExpr2 = new LambdaExpr(Token.NoToken, new List<TypeVariable>(), new List<Variable>() {boundVar}, null,
        id2);
