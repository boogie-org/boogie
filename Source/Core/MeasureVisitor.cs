...
namespace Microsoft.Boogie
{
    public class MeasureVisitor: StandardVisitor
    {
        ...
        private void CheckRecursiveProceduresHaveMeasure()
        {
            ...
            {
                var currentEdgeOrNull = callGraph.Edges.FirstOrDefault(e => e.Item1 == edge.Item1 && e.Item2 == edge.Item2, Tuple.Create < Implementation, Implementation > (null, null));
                if (currentEdgeOrNull != Tuple.Create < Implementation, Implementation > (null, null))
                {
                    ...
                    {
                        if (proc1 is YieldProcedureDecl yp && yp.MoverType.HasValue && yp.MoverType.Value == MoverType.Left)
                        {
                            if (proc1.Measure == null || proc1.Measure.Count == 0)
                            {
                                checkingContext.Error(tok1,  "Left-mover recursive procedures must have a measure annotation: {proc1.Name}");
                            }
                        }
                        else
                        {
                            checkingContext.Error(tok1, "Recursive procedures must have a measure annotation: {proc1.Name}");
                        }
                    }
                    if (alreadyChecked.Contains(impl2))
                    {
                        if (proc2 is YieldProcedureDecl yp2 && yp2.MoverType.HasValue && yp2.MoverType.Value == MoverType.Left)
                        {
                            if (proc2.Measure == null || proc2.Measure.Count == 0)
                            {
                                checkingContext.Error(tok2, "Left-mover recursive procedures must have a measure annotation: {proc2.Name}");
                            }
                        }
                        else
                        {
                            checkingContext.Error(tok2, "Recursive procedures must have a measure annotation: {proc2.Name}");
                        }
                    }
        ...
        public Implementation VisitImplementation2(Implementation impl)
        {
            ...
            foreach(var block in impl.Blocks)
            {
                ...
                {
                    newCmds.Add(cmd);
                    if (cmd is not CallCmd callCmd || callCmd.Proc == null || callCmd.Proc.Measure == null || impl.Proc.Measure == null || callCmd.Proc.Measure.Count == 0)
                    {
                        continue;
                    }
                    bool recursive = callGraph.Edges.Any(e => e.Item1 == impl && e.Item2.Proc == callCmd.Proc);
                    if (!recursive)
                    {
                        continue;
                    }
                    ...
                    {
                        throw new InvalidOperationException("Call to {callCmd.Proc.Name} has {callCmd.Ins.Count} actuals but {callCmd.Proc.InParams.Count} formals.");
                    }
              
                    var callFormalsToActuals = Substituter.SubstitutionFromDictionary(callCmd.Proc.InParams.Zip(callCmd.Ins, (formal, actual) => (formal, actual)).ToDictionary(p => (Variable) p.formal, p => (Expr) p.actual));
                    // ----------------------------------------------------------
                    ...
                    for (int i = 0; i < k; i++)
                    {
                        ...
                        Expr instantiated = Substituter.Apply(callFormalsToActuals, callCmd.Proc.Measure[i].Condition);
                        ...
                        equalPrefix = Expr.And(equalPrefix, Expr.Eq(instantiated, callerMeasure));
                    }
                    newCmds.Add(new AssertCmd(callCmd.tok, decreasing, new MeasureDescription(), null));
                }
                newBlocks.Add(new Block(block.tok, block.Label, newCmds, block.TransferCmd));
            }
