...
namespace Microsoft.Boogie
{
  public class FixedpointVC : VC.VCGen
  {
    ...
    public FixedpointVC(Program _program, string /*?*/ logFilePath, bool appendLogFile, List<Checker> checkers,
      Dictionary<string, int> _extraRecBound = null)
    ...
    public class LazyInliningInfo
    {
      ...
      public Hashtable /*Variable->Expr*/
        exitIncarnationMap;

      public Hashtable /*GotoCmd->returnCmd*/
        gotoCmdOrigins;
        ...
        GlobalVariable errorVariable)
      {
        ...
        {
          Contract.Assert(v != null);
          functionInterfaceVars.Add(new Formal(Token.NoToken, new TypedIdent(Token.NoToken, v.Name, v.TypedIdent.Type),
            true));
        }
        ...
        for (int i = 0; i < /* CommandLineOptions.Clo.ProcedureCopyBound */ 0; i++)
        {
          ...
          foreach (Variable v in interfaceVars)
          {
            Constant constant = new Constant(Token.NoToken,
              new TypedIdent(Token.NoToken, v.Name + temp++, v.TypedIdent.Type));
    ...
    public class StratifiedInliningInfo : LazyInliningInfo
    {
      ...
      public Hashtable /*<Block, LetVariable!>*/
      ...
      foreach (var impl in program.Implementations)
      {
...

        {
          StratifiedInliningInfo info =
            new StratifiedInliningInfo(impl, program, boogieContext, QuantifierExpr.GetNextSkolemId());
          ...
          else if (mode == Mode.Corral || proc.FindExprAttribute("inline") != null || proc is LoopProcedure)
          {
            ...
            if (mode == Mode.Corral || mode == Mode.OldCorral)
              proc.Ensures.Add(new Ensures(Token.NoToken, true, freePostExpr, "",
    ...
    private void ConjecturesToSpecs()
    {
      ...
      var twr = new TokenTextWriter(
        CommandLineOptions.Clo.PrintConjectures + "." + ConjectureFileCounter.ToString(), /*pretty=*/ false);
          ...
          if (lhs.GetKind() == TermKind.App)
          {
            ...
            if (r.GetKind() == DeclKind.Label)
            {
              ...
              if (!(arg.GetKind() == TermKind.App && arg.GetAppDecl().GetKind() == DeclKind.Uninterpreted))
                goto normal;
              if (!(annotationInfo.ContainsKey(arg.GetAppDecl().GetDeclName()) &&
                    annotationInfo[arg.GetAppDecl().GetDeclName()].type == AnnotationInfo.AnnotationType.LoopInvariant))
                goto normal;
    ...
    private void GenerateVCForStratifiedInlining(Program program, StratifiedInliningInfo info, Checker checker)
    {
      ...
      if (NoLabels)
      {
        ...
        VCExpr
          startCorrect =
            null; /* VC.VCGen.LetVC(cce.NonNull(impl.Blocks[0]), null, null, info.blockVariables, info.bindings,
