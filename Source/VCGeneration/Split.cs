...
namespace VC
{
  ...
  public class Split : ProofRun
  {
      ...
      public Split(VCGenOptions options, List<Block /*!*/> /*!*/ blocks,
        ...
        VCGen /*!*/ par, Implementation /*!*/ implementation, ImplementationRun run)
      {
      ...
      Split DoSplit()
      {
        ...
        return new Split(options, newBlocks, newGotoCmdOrigins, parent, Implementation, run);
      }
      ...
      public static List<Split /*!*/> FindManualSplits(Split initialSplit, ImplementationRun run, bool splitOnEveryAssert)
      {
        ...
        else
        {
          ...
          splits.Add(new Split(initialSplit.options, baseSplitBlocks, initialSplit.gotoCmdOrigins, initialSplit.parent, initialSplit.Implementation, run));
          foreach (KeyValuePair<Block, int> pair in splitPoints)
          {
            for (int i = 0; i < pair.Value; i++)
            {
              ...
              splits.Add(new Split(initialSplit.options, PostProcess(newBlocks), initialSplit.gotoCmdOrigins, initialSplit.parent, initialSplit.Implementation, run)); // REVIEW: Does gotoCmdOrigins need to be changed at all?
            }
          }
        }
        return splits;
      ...
      public static List<Split> FocusImpl(VCGenOptions options, Implementation impl, ImplementationRun run, Dictionary<TransferCmd, ReturnCmd> gotoCmdOrigins, VCGen par)
      {
        ...
        if (!focusBlocks.Any()) {
          ...
          f.Add(new Split(options, impl.Blocks, gotoCmdOrigins, par, impl, run));
          return f;
        }
        ...
        void FocusRec(int focusIdx, IEnumerable<Block> blocks, IEnumerable<Block> freeBlocks)
        {
          if (focusIdx == focusBlocks.Count())
          {
            ...
            s.Add(new Split(options, PostProcess(newBlocks), gotoCmdOrigins, par, impl, run));
          }
      ...
      public static List<Split> FocusAndSplit(VCGenOptions options, Implementation impl, ImplementationRun run, Dictionary<TransferCmd, ReturnCmd> gotoCmdOrigins, VCGen par, bool splitOnEveryAssert)
      {
        List<Split> focussedImpl = FocusImpl(options, impl, run, gotoCmdOrigins, par);
        var splits = focussedImpl.Select(s => FindManualSplits(s, run, splitOnEveryAssert)).SelectMany(x => x).ToList();
        return splits;
      }

      public static List<Split /*!*/> /*!*/ DoSplit(Split initial, ImplementationRun run, double splitThreshold, int maxSplits)
      {
