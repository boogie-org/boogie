...
namespace VC
{
  ...
  public class StratifiedVCGen : StratifiedVCGenBase
  {
    ...
    public class ApiChecker
    {
      ...
      public Outcome CheckAssumptions(List<VCExpr> hardAssumptions, List<VCExpr> softAssumptions)
      {
        ...
        ProverInterface.Outcome outcome =
          prover.CheckAssumptions(hardAssumptions, softAssumptions, out unsatisfiedSoftAssumptions, reporter);
        return ConditionGeneration.ProverInterfaceOutcomeToConditionGenerationOutcome(outcome);
      }
    ...
    public class VerificationState
    {
      ...
      public VerificationState(VCExpr vcMain, FCallHandler calls, ProverInterface prover,
        ProverInterface.ErrorHandler reporter)
      {
    ...
    public override Outcome FindLeastToVerify(Implementation impl, ref HashSet<string> allBoolVars)
    {
      ...
      FCallHandler calls =
        new FCallHandler(prover.VCExprGen, implName2StratifiedInliningInfo, impl.Name, mainLabel2absy);
    ...
    private HashSet<VCExprVar> refinementLoop(ApiChecker apiChecker, HashSet<VCExprVar> trackedVars,
      HashSet<VCExprVar> trackedVarsUpperBound, HashSet<VCExprVar> allVars)
    {
      ...
      // First half
      var fh = new HashSet<VCExprVar>(trackedVars);
      fh.UnionWith(part2);
      var s1 = refinementLoop(apiChecker, fh, trackedVarsUpperBound, allVars);

      var a = new HashSet<VCExprVar>(part1);
      a.IntersectWith(s1);
      var b = new HashSet<VCExprVar>(part1);
      b.ExceptWith(s1);
      var c = new HashSet<VCExprVar>(trackedVarsUpperBound);
      c.ExceptWith(b);
    ...
    private bool refinementLoopCheckPath(ApiChecker apiChecker, HashSet<VCExprVar> varsToSet,
      HashSet<VCExprVar> allVars)
    {
    ...
    private Outcome stratifiedStep(int bound, VerificationState vState, HashSet<int> block)
    {
      ...
      foreach (int id in calls.currCandidates)
      {
        ...
        if (idBound <= bound &&
            (CommandLineOptions.Clo.StackDepthBound == 0 || sd <= CommandLineOptions.Clo.StackDepthBound))
        {
          if (idBound > 1)
    ...
    private void DoExpansion(List<int> /*!*/ candidates, VerificationState vState)
    {
      ...
      foreach (int id in candidates)
      {
        ...
        VCExprSubstitution substForall =
          new VCExprSubstitution(substForallDict, new Dictionary<TypeVariable, Microsoft.Boogie.Type>());
        ...
        foreach (VCExprVar v in info.privateExprVars)
        {
          ...
          newVarCnt++;
          prover.Context.DeclareConstant(new Constant(Token.NoToken, new TypedIdent(Token.NoToken, newName, v.Type)),
            false, null);
        ...
        VCExprSubstitution substExists =
          new VCExprSubstitution(substExistsDict, new Dictionary<TypeVariable, Microsoft.Boogie.Type>());
    ...
    {
      Dictionary<string /*!*/, StratifiedInliningInfo /*!*/> /*!*/
        implName2StratifiedInliningInfo;

      public readonly Dictionary<int, Absy> /*!*/
        mainLabel2absy;

      public Dictionary<BoogieCallExpr /*!*/, int> /*!*/
        boogieExpr2Id;

      public Dictionary<BoogieCallExpr /*!*/, VCExpr> /*!*/
        recordExpr2Var;

      public Dictionary<int, VCExprNAry /*!*/> /*!*/
        id2Candidate;

      public Dictionary<int, VCExprVar /*!*/> /*!*/
        id2ControlVar;
    ...
    public class StratifiedInliningErrorReporter : ProverInterface.ErrorHandler
    {
      ...
      private void GetModelWithStates(Model m)
      {
        ...
        for (int i = 0; i < this.orderedStateIds.Count; ++i)
        {
          ...
          var prevInc = (lastCapturePoint != CALL && lastCapturePoint != RETURN && candidate == lastCandidate)
            ? info.CapturePoints[lastCapturePoint].IncarnationMap
          ...
          foreach (Variable v in info.AllVariables)
          {
            ...
            if (lastCapturePoint != CALL && lastCapturePoint != RETURN && prevInc[v] == e)
        ...
        int candidateId, Implementation procImpl, ModelViewInfo mvInfo)
      {
        ...
        Counterexample newCounterexample = GenerateTraceRec(labels, errModel, mvInfo, candidateId, entryBlock,
          traceNodes, trace, calleeCounterexamples);
        ...
        while (true)
        {
          ...
          for (int i = 0; i < cmds.Count; i++)
          {
                ...
                (cmd is AssumeCmd && QKeyValue.FindBoolAttribute((cmd as AssumeCmd).Attributes, "exitAssert")))
            {
              ...
              if (acmd == null)
              {
                acmd = new AssertCmd(Token.NoToken, Expr.True);
              }
            ...
            else
            {
              ...
              calleeCounterexamples[new TraceLocation(trace.Count - 1, i)] =
                new CalleeCounterexampleInfo(
                  GenerateTrace(labels, errModel, calleeId, calleeInfo.impl, calleeInfo.mvInfo), new List<object>());
