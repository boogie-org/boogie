...
public class YieldingProcDuplicator : Duplicator
{
  ...
  private Graph<Implementation> transitiveCallGraph;
  private HashSet<Implementation> recursiveImpls;
  ...
  public YieldingProcDuplicator(CivlTypeChecker civlTypeChecker, int layerNum, bool doRefinementCheck)
  {
    ...
    var program = civlTypeChecker.program;

// map procedures to implementations (only those that actually have bodies)
    procToImpl = program.Implementations.ToDictionary(impl => impl.Proc, impl => impl);

// build transitive call graph (this is what detects indirect recursion)
    transitiveCallGraph = Program.BuildTransitiveCallGraph(civlTypeChecker.Options, program);

// cache which impls are recursive (self-edge in transitive closure)
    recursiveImpls = new HashSet<Implementation>(transitiveCallGraph.Edges
  ...
  public override Procedure VisitYieldProcedureDecl(YieldProcedureDecl node)
  {
    if (!procToDuplicate.ContainsKey(node))
    {
      ...
      bool isRecursive = procToImpl.TryGetValue(node, out var impl) && recursiveImpls.Contains(impl);

      bool isLeftMoverAtDeclLayer =
        node.MoverType.HasValue &&
        node.MoverType.Value == MoverType.Left &&
        layerNum == node.Layer &&           // only enforce on the “main” desugaring layer
        !doRefinementCheck;                 // avoid duplicate errors for refine copies

      if (isRecursive && isLeftMoverAtDeclLayer && (node.Measure == null || node.Measure.Count == 0))
      {
        // Use the ORIGINAL token + ORIGINAL name 
        civlTypeChecker.checkingContext.Error(
          node.tok,
          $"left-mover recursive yielding procedure must have a measure annotation: {node.Name}"
        );
      }
    ...
    if (doRefinementCheck)
    {
      if (!noRequiresPureProcedures.TryGetValue(newCall.Proc.Name, out Procedure checker))
      {
        checker = DeclHelper.Procedure(
          ...
          newCall.Proc.InParams, newCall.Proc.OutParams,
          [], [], new List<Ensures>(newCall.Proc.Ensures), new List<Measure>(newCall.Proc.Measure), []);
