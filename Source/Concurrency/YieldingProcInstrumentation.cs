  ...
  class YieldingProcInstrumentation
  {
      ...
      bool doRefinementCheck)
    {
      ...
      wrapperYieldToYieldNoninterferenceCheckerProc = DeclHelper.Procedure(
        ...
        wrapperNoninterferenceCheckerFormals, new List<Variable>(), new List<Requires>(), new List<Requires>(), new List<Ensures>(), new List<Decreases>(), new List<IdentifierExpr>());
    ...
    private void DesugarParCallCmdInBlock(Block block, bool isBlockInYieldingLoop)
    {
      ...
      if (!parallelCallAggregators.ContainsKey(procName))
      {
        ...
        List<Decreases> decreasesSeq = new List<Decreases>();
        int count = 0;
        foreach (CallCmd callCmd in parCallCmd.CallCmds)
        {
          ...
          foreach (Decreases ens in callCmd.Proc.Decreases)
          {
            decreasesSeq.Add(new Decreases(ens.tok, ens.Free, Substituter.Apply(subst, ens.Condition), null,
              ens.Attributes));
          }
        ...
        parallelCallAggregators[procName] = DeclHelper.Procedure(
          ...
          ensuresSeq, decreasesSeq, civlTypeChecker.GlobalVariables.Select(v => Expr.Ident(v)).ToList());
      }
    ...
    private IEnumerable<Declaration> ActionNoninterferenceCheckers(IEnumerable<Action> actions, bool isGlobal)
    {
      ...
      foreach (var action in actions.Where(action => action.ModifiedGlobalVars.Count > 0))
      {
        ...
        var decreases = new List<Decreases>();
        ...
        var proc = DeclHelper.Procedure(name, inputs, outputs, requires, preserves, ensures, decreases, modifies);
