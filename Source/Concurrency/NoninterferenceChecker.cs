...
namespace Microsoft.Boogie
{
  public static class NoninterferenceChecker
  {
      ...
      List<Variable> declLocalVariables)
    {
      ...
      {
        yieldInfo = CollectYields(civlTypeChecker, absyMap, layerNum, impl).Select(kv =>
          new Tuple<List<Cmd>, List<PredicateCmd>>(
            linearPermissionInstrumentation.DisjointnessAssumeCmds(kv.Key, false), kv.Value)).ToList();
      }
