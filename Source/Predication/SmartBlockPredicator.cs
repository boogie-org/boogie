...
namespace Microsoft.Boogie
{
  public class SmartBlockPredicator
  {
        ...
        if (predMap.ContainsKey(n.Item1))
        {
          ...
          if (n.Item2)
          {
            ...
            if (dominator != null && predMap.ContainsKey(dominator))
            {
              ...
              aCmd.Attributes = new QKeyValue(Token.NoToken, "dominator_predicate",
                new List<object>() {predMap[dominator].ToString()}, aCmd.Attributes);
              aCmd.Attributes = new QKeyValue(Token.NoToken, "predicate",
                new List<object>() {predMap[n.Item1].ToString()}, aCmd.Attributes);
    ...
    private void PredicateBlock(Expr pExpr, Block block, List<Block> newBlocks, ref Block prevBlock)
    {
      ...
      newBlocks.Add(block);
      if (prevBlock != null &&
          !((prevBlock.TransferCmd is ReturnCmd) && uni != null && uni.IsUniform(impl.Name, block)))
      {
        ...
        if (predMap.ContainsKey(parent))
        {
          ...
          if (parentPred != null)
          {
            ...
            block.Cmds.Add(new AssertCmd(Token.NoToken,
              pCurrentExpr != null
                ? (Expr) Expr.Imp(pCurrentExpr, pParentExpr)
                : pParentExpr));
          }
        }
        ...
        if (decl is Procedure || decl is Implementation)
        {
          ...
          if (upp)
          {
            ...
            if (impl == null)
            {
              ...
              {
                new EnabledReplacementVisitor(new IdentifierExpr(Token.NoToken, fpVar), Expr.True).VisitExpr(
                  e.Condition);
