...
invariant (forall pid: Pid, req: Request :: (Set_Contains(ParticipantIds(), pid) && participant_votes[pid][req->id] == YES()) ==> Set_Contains(locked_requests[pid], req)); // (9, 1) (9,2) (9,*)
...
invariant {:layer 1} (forall req:Request ::  Set_Contains(committed_requests, req)  ==> (forall pid: Pid :: (1 <= pid && pid <= NumParticipants) ==>  (Set_Contains(locked_requests[pid], req))));
...
//    assume false;
   ...
   if (d == COMMIT()) {
        ...
        // committed => locked
        assert {:layer 1} (forall req:Request ::  Set_Contains(committed_requests, req)  ==> (forall pid: Pid :: (1 <= pid && pid <= NumParticipants) ==>  (Set_Contains(locked_requests[pid], req))));

        // assert {:layer 1} !(exists req': Request :: Set_Contains(committed_requests, req') && req'->duration == req->val->duration);
        ...
        // assert {:layer 1} !(exists req1: Request, req2: Request :: req1 != req2 && req1->duration == req2->duration && Set_Contains(committed_requests, req1) && Set_Contains(committed_requests, req2));
   }
}
