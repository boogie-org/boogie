...
creates read_f;
{
    ...
    call create_asyncs((lambda pa:read_f :: (1 <= pa->i) && (pa->i <= n) && pa->perm == One(Permission(tid, pa->i))));
}
...
modifies r1, pSet;
{
    ...
    sps2 := sps;
    // call sps := Set_Get(pSet, (lambda x: Permission :: (x->t_id == tid) && (1 <= x->mem_index) && (x->mem_index <= n)));
    assume (sps == Set((lambda x: Permission :: (x->t_id == tid) && (1 <= x->mem_index) && (x->mem_index <= n))));
    havoc r1;
...
creates read_f, main_s;
{
    ...
    var choice: read_f;
    sps2 := sps;
    // call sps := Set_Get(pSet, Set(lambda x: Permission :: (x->t_id == tid) && (1 <= x->mem_index) && (x->mem_index <= n)));
    assume sps == Set((lambda x: Permission :: (x->t_id == tid) && (1 <= x->mem_index) && (x->mem_index <= n)));
    assume {:add_to_pool "A", j+1, 0} 0 <= j && j <= n;
    havoc r1;
    assume (forall i:int :: ((1 <= i && i <= j) ==> (r1[tid][i]->ts < mem[i]->ts || r1[tid][i]== mem[i]))); 
    call done_set := Set_Get(sps2, (lambda x: Permission :: (x->t_id == tid) && (1 <= x->mem_index) && (x->mem_index <= j))); 
    call Set_Put(pSet, sps2);
    if (j < n){
        choice := read_f(One(Permission(tid, j+1)), j+1);
        assume {:add_to_pool "C", choice} true;
        call create_asyncs((lambda {:pool "C" } pa:read_f :: ((j+1) <= pa->i) && (pa->i <= n) && pa->perm == One(Permission(tid, pa->i))));
