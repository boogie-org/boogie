...
var {:layer 0,2} done_first: [Tid]bool;
var {:layer 0,1} r1: [Tid][int]StampedValue;  
// var {:layer 0,2} done_second: [Tid]bool; 
...
async action {:layer 1} action_main_f(tid: Tid, {:linear_in} sps: Set Permission)
...
modifies r1, pSet, done_first;
{
    ...
    if (Set_IsSubset(WholeTidPermission(perm->val->t_id), pSet)){
            assert Set_IsSubset(WholeTidPermission(perm->val->t_id), pSet);
            assert done_first[perm->val->t_id] == false;
