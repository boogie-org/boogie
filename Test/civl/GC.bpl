...
requires (forall x: idx :: rootAddr(x) ==> rootAbs[x] == Int(0));
...
requires {:layer 97,98,99,100} tid == GcTid;
{
    ...
    while (*)
    invariant {:yields 95,96,97,98,99,100}
...
procedure {:yields} {:layer 98} {:refines "AtomicFindFreePtr"} FindFreePtr({:linear "tid"} tid: Tid, absPtr: obj) returns (ptr: int)
{
    ...
    spaceFound := false;
    while (true)
    invariant {:yields 95,96,97,98} true;
