...
procedure {:yields} {:layer 1} {:refines "AtomicRemoteIncrementBody"} RemoteIncrementBody(i: int, {:linear_in "addr"} l: Addr)
{
	...
	async call IncrementCallback(i, l');
    }
    yield;
}
...
procedure {:left} {:layer 4,5} AtomicLocalIncrement(i: int, {:linear_in "addr"} l: Addr)
modifies x;
{
    assert isAddrWhole(i, l);
    x[i] := x[i] + 1;
...
procedure {:yields} {:layer 4} {:refines "AtomicLocalMultiply"} IncrementCallback(i: int, {:linear_in "addr"} l: Addr)
{
    yield;
    call LocalMultiply(i, l);
    yield;
}
//// End increment procedures
...
procedure {:yields} {:layer 1} {:refines "AtomicRemoteMultiplyBody"} RemoteMultiplyBody(i: int, {:linear_in "addr"} l: Addr)
{
    ...
    if (b) {
    	...
    	async call MultiplyCallback(i, l');
    }
    yield;
}
...
modifies done, x, Addrs;
{
    ...
    x[i] := 2*x[i];
    Addrs[i] := false;
    async call MultiplyCallback(i, l);  // allowed even though MultiplyCallback disappears at layer 2
}
...
procedure {:left} {:layer 4,5} AtomicLocalMultiply(i: int, {:linear_in "addr"} l: Addr)
modifies x;
{
    assert isAddrWhole(i, l);
    x[i] := 2*x[i];
...
procedure {:both} {:layer 3} Skip(i: int, {:linear_in "addr"} l: Addr)
{
}
procedure {:yields} {:layer 2} {:refines "Skip"} MultiplyCallback(i: int, {:linear_in "addr"} l: Addr)
{
    yield;
}
//// End multiply procedures

//// Start main procedure
procedure {:both} {:layer 6} IncrementThenAdd(i: int, {:linear_in "addr"} l: Addr)
modifies x;
{
    assert isAddrWhole(i, l);
    x[i] := 2 * (x[i] + 1);
}
procedure {:yields} {:layer 5} {:refines "IncrementThenAdd"} Main(i: int, {:linear_in "addr"} l: Addr)
{
    yield;
    call LocalIncrement(i, l);
    yield;
}
//// End main procedure
