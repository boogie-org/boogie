...
atomic action {:layer 2} main_atomic ({:linear_in} perms: UnitMap (One Perm))
...
main ({:linear_in} perms: UnitMap (One Perm))
refines main_atomic;
requires {:layer 1} perms->dom == all_perms();
ensures call YieldAllDecided();
{
  var s:int;
  var perms': UnitMap (One Perm);
  var perms'': UnitMap (One Perm);
  s := 1;
  perms' := perms;
  while (s <= N)
  invariant {:layer 1} perms'->dom == s_perms_geq(s);
...
yield left procedure {:layer 1} P (s:int, {:linear_in} perms: UnitMap (One Perm))
requires {:layer 1} perms->dom == s_perms_eq(s);
...
modifies col_dom, col_val, dec_dom, dec_val;
{
  ...
  while (r <= N)
  invariant {:layer 1} perms'->dom == s_r_perms_geq(s,r);
...
yield both procedure {:layer 1} split_perms_sender (s:Pid, {:linear_in} perms_in: UnitMap (One Perm)) returns ({:linear} perms_out_1: UnitMap (One Perm), {:linear} perms_out_2: UnitMap (One Perm));
requires {:layer 1} perms_in->dom == s_perms_geq(s);
ensures {:layer 1} perms_out_1->dom == s_perms_geq(s+1);
ensures {:layer 1} perms_out_2->dom == s_perms_eq(s);

yield both procedure {:layer 1} split_perms_receiver (s:Pid, r:Pid, {:linear_in} perms_in: UnitMap (One Perm)) returns ({:linear} perms_out_1: UnitMap (One Perm), {:linear} perms_out_2: One Perm);
requires {:layer 1} perms_in->dom ==  s_r_perms_geq(s,r);
ensures {:layer 1} perms_out_1->dom == s_r_perms_geq(s,r+1);
