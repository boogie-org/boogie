...
// A parallel implementation of the prefix prescan problem
// using Blelloch's two-phase method comprising an upsweep
// followed by a downsweep.
...
yield left procedure {:layer 1} PrefixPreScan({:linear_in} A: Map (One int) int, N: int)
...
{
  call A' := UpSweep(A, N);
  call Path_Store(A'->val[One(PowerOfTwo(N) - 1)], 0);
  call A' := DownSweep(A', N);
}
...
yield left procedure {:layer 1} DownSweep({:linear_in} A: Map (One int) int, N: int)
returns ({:linear} A': Map (One int) int)
requires {:layer 1} 0 < N;
requires {:layer 1} (forall j: int:: 0 <= j && j < PowerOfTwo(N) ==> Map_Contains(A, One(j)));
ensures {:layer 1} A->dom == A'->dom;
{
  ...
  n := N;
  while (n > 0)
  invariant {:layer 1} n <= N;
  invariant {:layer 1} A->dom == A'->dom;
  {
    n := n - 1;
    call A' := DownSweepAtLevel(A', PowerOfTwo(n) - 1, n, N);
  }
}

yield left procedure {:layer 1} DownSweepAtLevel({:linear_in} A: Map (One int) int, i: int, level: int, N: int)
returns ({:linear} A': Map (One int) int)
requires {:layer 1} 0 <= i;
requires {:layer 1} ((i + 1 - PowerOfTwo(level)) mod (2 * PowerOfTwo(level))) == 0;
requires {:layer 1} 0 <= level && level < N;
...
ensures {:layer 1} A->dom == A'->dom;
{
  ...
  stride := PowerOfTwo(level);
  call B := Map_Split(A', Set_Add(Set_Singleton(One(i)), One(i + stride)));
  call B := DownSweepOp(B, i, stride) | A' := DownSweepAtLevel(A', i + 2 * stride, level, N);
  call Map_Join(A', B);
}

yield left procedure {:layer 1} DownSweepOp({:linear_in} B: Map (One int) int, i: int, stride: int)
returns ({:linear} B': Map (One int) int)
requires {:layer 1} Map_Contains(B, One(i)) && Map_Contains(B, One(i + stride));
ensures {:layer 1} B->dom == B'->dom;
{
