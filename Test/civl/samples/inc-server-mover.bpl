...
yield procedure {:layer 9} main ({:linear_in} p : One int)
...
yield left procedure {:layer 9} Server_Inc ({:linear_in} p : One int)
...
yield left procedure {:layer 9} AsyncInc ({:linear_in} p : One int)
requires {:layer 9} perm(p) && x == y;
ensures  {:layer 9} x == old(x) + 1;
modifies x;
{
...
left action {:layer 9} inc_x_perm_atomic ({:linear} p : One int)
modifies x;
{ assert perm(p); x := x + 1; }
