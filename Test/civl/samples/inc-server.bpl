...
yield procedure {:layer 11} main ({:linear_in} p : One int)
...
left action {:layer 10,11} inc_x_high_atomic ({:linear_in} p : One int)
modifies x;
{ x := x + 1; }

yield procedure {:layer 10} Server_Inc ({:linear_in} p : One int)
...
yield procedure {:layer 9} AsyncInc ({:linear_in} p : One int)
refines inc_x_high_atomic;
requires call Yield_9(p);
{
  call inc_x_perm(p);
...
atomic action {:layer 9} inc_x_perm_atomic ({:linear} p : One int)
modifies x;
{ assert perm(p); x := x + 1; }

left action {:layer 9} Client_IncDone_atomic ({:linear_in} p : One int)
{ assert perm(p) && x == y + 1; }

yield procedure {:layer 8} inc_x_perm ({:linear} p : One int)
refines inc_x_perm_atomic;
{
  call inc_x();
}

yield procedure {:layer 8} Client_IncDone ({:linear_in} p : One int)
