...
yield invariant {:layer 2} LocInStack(loc_t: LocTreiber X, loc_n: LocNode X);
preserves Map_Contains(TreiberPoolLow, One(loc_t));
preserves Set_Contains(Domain(TreiberPoolLow, loc_t), One(loc_n));

yield invariant {:layer 3} ReachInStack(loc_t: LocTreiber X);
preserves Map_Contains(TreiberPoolLow, One(loc_t));
...
yield invariant {:layer 3} StackDom();
preserves TreiberPool->dom == TreiberPoolLow->dom;

yield invariant {:layer 3} PushLocInStack(loc_t: LocTreiber X, node: Node X, new_loc_n: LocNode X, {:linear} tagged_loc: One (TaggedLocNode X));
preserves Map_Contains(TreiberPoolLow, One(loc_t));
...
atomic action {:layer 4} AtomicAlloc() returns ({:linear} tagged_loc: One (TaggedLocTreiber X))
{
...
yield procedure {:layer 3} Alloc() returns ({:linear} tagged_loc: One (TaggedLocTreiber X))
refines AtomicAlloc;
...
preserves call StackDom();
{
  ...
  call {:layer 3} TreiberPool := Copy(Map_Update(TreiberPool, one_loc_t, Vec_Empty()));
  call {:layer 3} AbsLemma(treiber);
}

atomic action {:layer 4} AtomicPush(loc_t: LocTreiber X, x: X) returns (success: bool)
{
  if (*) {
...
yield procedure {:layer 3} {:vcs_split_on_every_assert} Push(loc_t: LocTreiber X, x: X) returns (success: bool)
refines AtomicPush;
...
preserves call StackDom();
{
  ...
  var {:layer 3} old_treiber: Treiber X;

  call {:layer 3} old_treiber := Copy(TreiberPoolLow->val[One(loc_t)]);
  call loc_n, new_loc_n, tagged_loc := CreateNewTopOfStack(loc_t, x);
  call {:layer 3} FrameLemma(old_treiber, TreiberPoolLow->val[One(loc_t)]);
  ...
  if (success) {
    call {:layer 3} TreiberPool := Copy(Map_Update(TreiberPool, One(loc_t), Vec_Append(Map_At(TreiberPool, One(loc_t)), x)));
    assert {:layer 3} TreiberPoolLow->val[One(loc_t)]->top != None();
    call {:layer 3} AbsLemma(TreiberPoolLow->val[One(loc_t)]);
  }
}

atomic action {:layer 4} AtomicPop(loc_t: LocTreiber X) returns (success: bool, x_opt: Option X)
{
  var stack: Vec X;
...
yield procedure {:layer 3} Pop(loc_t: LocTreiber X) returns (success: bool, x_opt: Option X)
refines AtomicPop;
preserves call TopInStack(loc_t);
preserves call ReachInStack(loc_t);
preserves call StackDom();
{
  ...
  if (x_opt is Some) {
    assert {:layer 3} Vec_Len(Map_At(TreiberPool, One(loc_t))) > 0;
    call {:layer 3} TreiberPool := Copy(Map_Update(TreiberPool, One(loc_t), Vec_Remove(Map_At(TreiberPool, One(loc_t)))));
  }
}

atomic action {:layer 3} AtomicCreateNewTopOfStack(loc_t: LocTreiber X, x: X)
...
yield procedure {:layer 2} CreateNewTopOfStack(loc_t: LocTreiber X, x: X)
  returns (loc_n: Option (LocNode X), new_loc_n: LocNode X, {:linear} tagged_loc: One (TaggedLocNode X))
preserves call TopInStack(loc_t);
...
atomic action {:layer 3} AtomicPopIntermediate(loc_t: LocTreiber X) returns (success: bool, x_opt: Option X)
{
  var one_loc_t: One (LocTreiber X);
  var loc_n: Option (LocNode X);
  var treiber: Treiber X;
...
yield procedure {:layer 2} PopIntermediate(loc_t: LocTreiber X) returns (success: bool, x_opt: Option X)
